    ...
    if (activeTab === "Prévisionnels") {
      ...
      const fetchDailyReports = async () => {
        try {
          ...
          if (response.data.success) {
            console.log("Daily reports fetched successfully:", response.data);
            const reportData = response.data.data.map((report, index) => ({
              id: index + 1,
              identifier: report.reportName,
              well: report.concernedWell.nom,
              wellId: report.concernedWell.id.toString(),
            }));
            setReports(reportData);
          } else {
            throw new Error("API response indicates failure");
          ...
          setError(`Failed to fetch daily reports: ${error.message}`);
        } finally {
          setLoading(false);
        }
      };
